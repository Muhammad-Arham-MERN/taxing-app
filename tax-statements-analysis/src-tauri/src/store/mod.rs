...
/// Schema version stamped into `user_version`. No migration is implemented —
/// the developer deferred store upgrades to a later module — but the value is
/// written now so that decision has something to work with.
pub const SCHEMA_VERSION: i32 = 1;

/// The tables a store of ours must have.
const REQUIRED_TABLES: [&str; 3] = ["app_meta", "statements", "attachments"];
...
pub enum Verification {
    /// Ours. `empty` is true when it holds no statements (FR-064).
    Ours { empty: bool },
...
pub fn verify_read_only(path: &Path) -> StoreResult<Verification> {
    ...
    Ok(Verification::Ours { empty: count == 0 })
}
...
/// Open an existing store read-write after verifying it is ours.
